...
public class Main {
    ...
    public static void main(String[] args){
        ...
        try{
            ...
            while(in.hasNextLine()){
                ...
                //dC is a function I created to convert a String to a double (just to save some space)
                Manager currentManager = new Manager(mP[0],mP[1],mP[2],mP[3],dC(mP[4]),dC(mP[5]),dC(mP[6]),dC(mP[7]));
            ...
            while(in.hasNextLine()){
                ...
                //dC is a function I created to convert a String to a double (just to save some space)
                Player currentPlayer = new Player(pP[0],pP[1],pP[2],pP[3],dC(pP[4]),dC(pP[5]),dC(pP[6]),dC(pP[7]),dC(pP[8]),dC(pP[9]),dC(pP[10]),dC(pP[11]),dC(pP[12]),dC(pP[13]));

                //loop through the squads array:
                for(int i = 0; i < squads.length; i++){
                    String teamName = squads[i].getTeamName();//retrieve the team name
                    //if the team name matches the team name that of the player then add tha player to the squad
        ...
        //loop through all the squares to pick the best players and add them to the array above
        ...
        for(Team squad: tournamentSquads){
            System.out.println("Team:"+squad.getTeamName()+" Manager:"+squad.getManager().getSurname());
            for(int i = 0; i < 11; i++){
                System.out.println(squad.getPlayer(i).getSurname()+" "+squad.getPlayer(i).getPosition());
            }
        }*/
    ...
    public static void runTournament(ArrayList<Team> tournamentSquads){
        ...
        for(int i = 0; i < tournamentSquads.size(); i++){
            if((i+1)%4 == 0){
                ...
                Team[] returned = groupDivision(tournamentSquads.get(i-3), tournamentSquads.get(i-2), tournamentSquads.get(i-1), tournamentSquads.get(i));
    ...
    public static String playMatch(Team t1, Team t2, boolean canDraw){
       ...
       //calculate the chance of goal:
        ...
        //match result:
        System.out.println("Match Result: "+t1.getTeamName()+" "+team1Goals+"   "+team2Goals+" "+t2.getTeamName());
        //GroupStage:
        if(canDraw){
        ...
        //After Group Stage:
            ...
            if(team1Goals > team2Goals){
                ...
                System.out.println("Penalty result: "+t1.getTeamName()+" "+t1PenaltiesMade+"   "+t2.getTeamName()+" "+t2PenaltiesMade);
    ...
    private static boolean shotAttempt(int t1ChancesToScore, int t2ChanceToSave, Team t) {
        Random random = new Random();
        String[] randomPhrase = {" gets a shot on goal!"," takes a shot!", " fires it towards the goal!", " has a chance to score!"};
        int x = random.nextInt(100);//0-99
        //shot attempt:
    ...
    private static double chancesToSave(Team t) {
        ...
        double managersBonus = manager.getAbility()+ manager.getRespect()+ manager.getBelief()+ manager.getKnowledge();
        total+=managersBonus;
    ...
    private static double chancesToScore(Team t) {
        //In this case I only take into consideration the midfielders skill level and the forwards skill level
        double total = 0;
